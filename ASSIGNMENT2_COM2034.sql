...
GO
ALTER TRIGGER TR_BAI2 ON ChiTietSP FOR INSERT, UPDATE
AS
...
GO
CREATE TRIGGER TR_DELETE ON NhanVien
INSTEAD OF DELETE
AS
BEGIN
...
END
--DELETE FROM NhanVien WHERE Ma LIKE 'NV1'

--BÀI 3:
--Viết hàm tính tuổi tuyệt đối với tham số đầu vào là date. Hãy viết hàm tính tuổi
GO
...
--bài 3.3
GO 
CREATE FUNCTION TONG_TIEN(@masp NVARCHAR(20),@NG_BĐ DATE,@NG_KT DATE)
RETURNS NVARCHAR(50)
BEGIN
    ...
    ELSE 
	BEGIN
            RETURN CAST(@TIEN as nvarchar(20))+ ' ' + @masp +(select Ten
            from SanPham
            where @masp = SanPham.Ma)+ ' ' + cast(( SELECT SUM(SoLuong)
            FROM SanPham JOIN ChiTietSP on SanPham.Id = ChiTietSP.IdSP join HoaDonChiTiet on HoaDonChiTiet.IdChiTietSP = ChiTietSP.Id
            WHERE Ma = @masp
            GROUP BY Ma ) as nvarchar(30)) + ' ' + cast( ( SELECT SUM(SoLuong*GiaNhap)
            FROM SanPham JOIN ChiTietSP on SanPham.Id = ChiTietSP.IdSP join HoaDonChiTiet on HoaDonChiTiet.IdChiTietSP = ChiTietSP.Id
            WHERE Ma = @masp
...
BEGIN
	...
	IF @ID IS NULL OR @MA IS NULL OR @Ten IS NULL OR @TenDem IS NULL OR @Ho IS NULL OR @GioiTinh IS NULL OR @NgaySinh IS NULL OR 
		@DiaChi IS NULL OR @sdt IS NULL OR  @maCh IS NULL OR  @maCv IS NULL OR  @idGuiBC IS NULL OR  @trangThai IS NULL 
...
EXEC SP_PROC_NV @ID = 0,@Ma = '',@Ten = ' ',@Tendem = '',@Ho = '',@GioiTinh= '',@NgaySinh = '',@DiaChi='',@sdt = '',@maCh = '',@maCv = '',@idGuiBC = null,@trangThai= 1,@SQLTYPE = 'select'

EXEC SP_PROC_NV @ID = 30,@Ma = 'NV30',@Ten = N'Khánh',@Tendem = N'Ngọc',@Ho = N'Đỗ',@GioiTinh= 'Nam',@NgaySinh = '2002-10-07',@DiaChi=N'Thanh Hoá',@sdt = '0971063180',@maCh = 'CH10',@maCv = 'NV',@idGuiBC = null,@trangThai= 1,@SQLTYPE = 'INSERT'
EXEC SP_PROC_NV @ID = 31,@Ma = 'NV31',@Ten = N'Nam',@Tendem = N'Phương',@Ho = N'Đỗ',@GioiTinh= 'Nam',@NgaySinh = '2002-10-07',@DiaChi=N'Thanh Hoá',@sdt = '0971063180',@maCh = 'CH10',@maCv = 'NV',@idGuiBC = null,@trangThai= 1,@SQLTYPE = 'INSERT'
...
EXEC SP_KHACH_HANG @ID =22,@MA ='KH22',@TEN=N'Khánh',@TEN_DEM = 'Ngọc',@HO = N'Đỗ',@NGAY_SINH = '2001-10-07',@SDT = '0971063180',@DIACHI=N'Thanh Hoá',@THANH_PHO =N'Thanh Hoá',@QUOC_GIA =N'Việt Nam',@SQLTYPE ='insert'
---CÂU 3
...
BEGIN
	...
	IF @SQLTYPE = 'INSERT'
	BEGIN
	IF dbo.IDSP(@MASP) IS NULL OR dbo.IDNSX(@MANSX)  IS NULL OR dbo.IDMS(@MAMS) IS NULL OR dbo.IDDSP(@MADSP) IS NULL OR
	@NAMBH IS NULL OR @MOTA IS NULL OR @SL_TON IS NULL OR @GIA_NHAP IS NULL OR @GIA_BAN IS NULL
	PRINT N'VUI LÒNG NHẬP ĐỦ DỮ LIỆU'
	ELSE
	BEGIN TRY
		INSERT INTO KhachHang
...
EXEC SP_CT_SP @ID =0,@MASP ='',@MANSX='',@MAMS='',@MADSP='',@NAMBH=null,@MOTA='',@SL_TON ='',@GIA_NHAP =0,@GIA_BAN =0,@SQLTYPE = 'select'
